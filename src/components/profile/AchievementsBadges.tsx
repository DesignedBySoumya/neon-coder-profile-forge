...
const badges = [
  { name: 'First Blood', icon: '🥇', tier: 'gold', unlocked: true, description: 'Solved first problem' },
  { name: 'Speed Demon', icon: '⚡', tier: 'platinum', unlocked: true, description: 'Solved in under 2 minutes' },
  { name: 'Night Owl', icon: '🦉', tier: 'silver', unlocked: true, description: 'Solved problems after midnight' },
  { name: 'Marathon', icon: '🏃', tier: 'gold', unlocked: true, description: '100+ day streak' },
  { name: 'Algorithm Master', icon: '🧠', tier: 'platinum', unlocked: false, description: 'Master all algorithm categories' },
  { name: 'Code Reviewer', icon: '📋', tier: 'bronze', unlocked: true, description: 'Reviewed 50+ submissions' }
];
        ...
        {badges.map((badge, index) => (
          <div
            ...
            className={`group relative p-4 rounded-lg border transition-all duration-300 cursor-pointer ${
              badge.unlocked
                ? 'bg-[#0D0D0D] border-gray-700 hover:border-[#00FFA3]/50 hover:shadow-lg hover:shadow-[#00FFA3]/20'
