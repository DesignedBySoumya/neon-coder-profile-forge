  ...
  const generateContributions = (year: number) => {
    ...
    for (let week = 0; week < 52; week++) {
      for (let day = 0; day < 7; day++) {
        ...
        const count = date.getFullYear() === year && date <= endDate 
          ? Math.floor(Math.random() * 12) // Random contribution count
    ...
    <div className="bg-[#1A1A1A] rounded-xl p-6 border border-gray-800">
      <div className="flex items-center justify-between mb-6">
        ...
        <select 
          ...
          className="bg-[#0D0D0D] border border-gray-700 rounded-lg px-3 py-1 text-white text-sm focus:border-[#00FFA3]/50 focus:outline-none"
        >
      ...
      <div className="contribution-heatmap">
        {/* Month Labels */}
        <div className="grid grid-cols-52 gap-[2px] mb-2 text-xs text-[#A0A0A0]">
        ...
        <div className="flex gap-2">
          ...
          <div className="flex flex-col gap-[2px] text-xs text-[#A0A0A0] justify-center">
            {dayLabels.map((day, index) => (
              <div key={day} className="h-[10px] flex items-center" style={{ marginTop: index === 0 ? '0' : '10px' }}>
                {day}
              </div>
            ))}
          </div>
